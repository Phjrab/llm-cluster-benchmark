...
노드 화면은 Jetson을 먼저, Raspberry Pi를 다음에 배치하고 각 플랫폼 안에서는 워커 이름의
자연 숫자순으로 표시한다. 이 표시 정렬은 저장된 실험의 명시적 실행 순서를 바꾸지 않는다.
...
플랫폼별 llama-cpp-python 빌드 → pinned llama.cpp RPC 확인·필요 시 빌드 → 선택 모델 동기화
→ Worker API 시작이다. 이미 검증된 Python/RPC 환경은 재설치·재빌드하지 않고 재사용한다. 대시보드는
...
Worker 상세 화면의 **연결 해제**는 기본적으로 인벤토리 등록만 제거하고 원격 파일을
보존한다. 사용자가 **워커에 설치된 프로젝트 파일도 영구 삭제**를 별도로 선택하면,
Controller는 실행 중 실험·제어 작업이 없는지 확인하고 Worker API를 정상 종료한 다음
인벤토리에 기록된 단 하나의 프로젝트 경로만 삭제한다. 이 옵션은 코드, `.venv`, 모델,
로그와 워커 로컬 결과까지 복구 불가능하게 제거한다. 원격 경로가 symlink이거나 SSH 사용자
소유가 아니거나 실제 경로가 인벤토리와 다르면 삭제하지 않고 노드 등록도 유지한다.
