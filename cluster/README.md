...
토폴로지에는 head와 사용자가 직접 등록한 worker만 표시된다. 미리 만든
`1대 / 2대 / 4대` 구성은 없으며, 등록된 노드의 체크박스로 실험 참여 대상을
자유롭게 고른다. 목록 끝의 `+ 워커 추가` 카드나 상단의 `+ 워커 연결`을 누르면
head가 연결된 사설 LAN의 최대 `/24` 범위에서 SSH 포트가 열린 기기만 제한적으로
찾는다. Docker·가상 브리지 인터페이스는 제외한다. Head를 포함해 최대 4대까지
등록할 수 있다.
