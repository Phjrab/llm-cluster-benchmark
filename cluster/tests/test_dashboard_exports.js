...
assert.equal(vm.runInContext(`ClusterDashboard.paginateItems([1,2,3], 99, 2).page`, context), 2);
assert.equal(vm.runInContext(`ClusterDashboard.orbitWorkerState({enabled:true}, {api:true})`, context), "ONLINE");
assert.equal(vm.runInContext(`ClusterDashboard.orbitWorkerState({enabled:true}, {api:false})`, context), "OFFLINE");
assert.equal(vm.runInContext(`ClusterDashboard.orbitWorkerState({enabled:false}, {api:true})`, context), "DISABLED");
...
assert.match(modelsSource, /license-acceptance/);
assert.match(modelsSource, /license_fingerprint: status\.fingerprint/);
assert.match(modelsSource, /\/api\/huggingface\/status/);
assert.match(modelsSource, /Controller 인증 다운로드/);
assert.doesNotMatch(modelsSource, /access_token|authorization:\s*bearer|"token"\s*:/i);
assert.match(template, /id="modelStarterPacks"/);
assert.match(template, /id="modelStarterPackPreview"/);
assert.match(template, /id="modelPageSize"[\s\S]*id="modelPageButtons"/);
assert.match(template, /id="resultPageSize"[\s\S]*id="resultPageButtons"/);
assert.match(template, /id="orbitWorkers"/);
assert.doesNotMatch(template, /sat-one|sat-two|sat-three/);
...
const packPreview = vm.runInContext(`ClusterDashboard.modelLibrary.packPreviewData(
  { id: "smoke", model_ids: ["qwen-a", "missing-model"] },
  [{ id: "qwen-a", filename: "qwen.gguf", installed_nodes: ["jetson-1"], catalog: { display_name: "Qwen A", vendor: "Qwen", family: "Qwen2.5", parameters_total_b: 1.5 } }]
)`, context);
assert.equal(packPreview.entries.length, 2);
assert.deepEqual([...packPreview.available], ["qwen-a"]);
assert.equal(packPreview.entries[0].installed, true);
assert.equal(packPreview.entries[1].installed, false);
...
assert.match(researchSource, /formal_execution_allowed/);
assert.match(researchSource, /execution \$\{executionAllowed \? "open" : "blocked"\}/);
assert.match(researchSource, /data-campaign-action="start"/);
assert.match(researchSource, /data-campaign-action="retry"/);
assert.match(researchSource, /formal_execution_allowed === true/);
assert.match(researchSource, /confirmed: true/);
...
assert.match(appSource, /JETSON POWER MODE/);
